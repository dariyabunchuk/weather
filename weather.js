...
// Current icon
  icon_html_current = '<h3>' + icon(weather_current.icon) + '</h3>';
  $("#current-conditions-icon").append(icon_html_current);

// Current summary
  summary_html_current = '<h5>' + weather_current.summary + '</h5>';
  $("#current-conditions-text").append(summary_html_current);

// Loop for forecast
  for (let i=0; i<6; i++) {
    let html = '<div class="col">';
    html = html + '<h3>' + icon(weather[i].icon) + '</h3>';
    html = html + '<h4>' + Math.round(weather[i].temperatureMax) + " | " + Math.round(weather[i].temperatureMin) + '</h4>';
    html = html + '<h5>' + weather[i].summary + '</h5>';
    html = html + '</div>';
